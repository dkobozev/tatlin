import array
import unittest
from tatlin.lib.model.gcode.parser import (
    GcodeParser,
    GcodeLexer,
    GcodeParserError,
    Movement,
...
class GcodeParserTest(unittest.TestCase):
    ...
    def test_movement(self):
        m = Movement(array.array("f", [0, 0.5, 0]), 0, 0, 0)
        self.assertEqual(str(m), "(array('f', [0.0, 0.5, 0.0]))")
        self.assertEqual(repr(m), "Movement(array('f', [0.0, 0.5, 0.0]), 0, 0, 0)")
    ...
    def test_slic3r(self):
        gcode = """
        ; generated by Slic3r 0.6.1-dev on 2012-02-25 at 07:01:16
        G1 Z0.400 F12000.000 ; move to next layer
        G1 X78.810 Y79.100 ; move to first skirt point
        G1 F600.000 E1.00000 ; compensate retraction
        G1 X79.600 Y78.350 F1482.000 E1.02761 ; skirt
        G1 X92.780 Y92.780 F12000.000 ; move to first perimeter point
        """
        self.parser.load(gcode)
        self.parser.parse()

    def test_invalid_gcode(self):
        with self.assertRaises(GcodeParserError):
            self.parser.load("")
            self.parser.parse()
    ...
    def test_flags(self):
        ...
        G20 ; inches
        G28 ; move to origin
        G28 X0 Y0 Z0 ; move to origin with coords
        G91 ; relative positioning
        G92 ; set position to 0, 0, 0
        G1 Z0.400 F12000.000
        G1 X78.810 Y79.100
        G1 F600.000 E1.00000
        G1 X79.600 Y78.350 F1482.000 E1.02761
        """
        self.parser.load(gcode)
        self.parser.parse()
