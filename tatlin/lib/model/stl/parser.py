...
class StlAsciiParser(object):
    ...
    def _solid(self):
        ...
        if line[0] != "solid":
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("solid", line[0])
            )
    ...
    def _endsolid(self):
        ...
        if line[0] != "endsolid":
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("endsolid", line[0])
            )
    ...
    def _facet(self):
        ...
        if line[0] != "facet":
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("facet", line[0])
            )
        ...
        if line[1] == "normal":
            ...
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("normal", line[1])
            )
    ...
    def _endfacet(self):
        ...
        if line[0] != "endfacet":
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("endfacet", line[0])
            )
    ...
    def _outer_loop(self):
        ...
        if " ".join(line) != "outer loop":
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("outer loop", " ".join(line))
            )
    ...
    def _endloop(self):
        ...
        if line[0] != "endloop":
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("endloop", line[0])
            )
    ...
    def _vertex(self):
        ...
        if line[0] != "vertex":
            raise InvalidTokenError(
                self.line_no, 'expected "%s", got "%s"' % ("vertex", line[0])
            )
